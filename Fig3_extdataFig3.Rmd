...
```
p1<-ggplot(treated_samples[treatment=="Clitocine" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
          ...
          p2<-ggplot(treated_samples[treatment=="SRI" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
          ...
          p3<-ggplot(treated_samples[treatment=="G418" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
          ...
          p4<-ggplot(treated_samples[treatment=="DAP" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
          ...
          p5<-ggplot(treated_samples[treatment=="SJ6986" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
          ...
          p6<-ggplot(treated_samples[treatment=="CC90009" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
          ...
          p7<-ggplot(treated_samples[treatment=="FUr" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() + 
          ...
          p8<-ggplot(treated_samples[treatment=="Gentamicin" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() + 
          ...
          p9<-ggplot(treated_samples[treatment=="Untreated" & replicate==1 & GENEINFO=="TP53" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() + 
...
```
p3<-ggplot(treated_samples[treatment!="Untreated" & replicate==1 & reads_allbins>10 & GENEINFO=="LAMB3"], aes(y=RT, x=treatment)) + geom_boxplot(fill="#FFCCCC", alpha=.5, outlier.shape = NA) + 
      ...
      p4<-ggplot(treated_samples[treatment!="Untreated" & replicate==1 & reads_allbins>10 & GENEINFO=="CFTR"], aes(y=RT, x=treatment)) + geom_boxplot(fill="#FFCCCC", alpha=.5, outlier.shape = NA) + 
...
```
#### APC:
    tmp_1<-treated_samples[GENEINFO=="APC" & replicate==1 & reads_allbins>=10]
...
#### CFTR:  
    tmp_2<-treated_samples[GENEINFO=="CFTR" & replicate==1 & reads_allbins>=10]
...
#### DMD:
    tmp_3<-treated_samples[GENEINFO=="DMD" & replicate==1 & reads_allbins>=10, c("nt_seq", "RT", "treatment")]
...
**Ext.Data.Fig.3a:**
```
  ggplot(treated_samples[reads_allbins>=10]) + geom_hex(aes(x=RT, y=tAI_up_5aas), bins=70) + geom_smooth(aes(x=RT, y=tAI_up_5aas), method='lm') + stat_cor(aes(x=RT, y=tAI_up_5aas, label = ..r.label..), size=4, label.y.npc=c(1,1)) + facet_wrap(~treatment, nrow=3, scales = "free") + labs(y="tAI 5 codons upstream", x="Readthrough (%)") +
...
```
        ggplot(treated_samples[reads_allbins>=10]) + geom_hex(aes(x=RT, y=CAI_up_5aas), bins=70) + geom_smooth(aes(x=RT, y=CAI_up_5aas), method='lm') + stat_cor(aes(x=RT, y=CAI_up_5aas, label = ..r.label..), size=4, label.y.npc=c(1,1)) + facet_wrap(~treatment, nrow=3, scales = "free") + labs(y="CAI 5 codons upstream", x="Readthrough (%)") +
            scale_fill_viridis(limits = c(0, 100), oob = scales::squish, alpha = 0.85) + p
```
...
```               
        ggplot(treated_samples[reads_allbins>=10]) + geom_point(aes(x=RT, y=GCperc_5aas), alpha=.5, color="#FFCCCC") + geom_smooth(aes(x=RT, y=GCperc_5aas), method='lm') + stat_cor(aes(x=RT, y=GCperc_5aas, label = ..r.label..), size=4, label.y.npc=c(1,1)) + facet_wrap(~treatment, nrow=3, scales = "free") + labs(y="GC% 5 codons upstream", x="Readthrough (%)") +
            scale_fill_viridis(limits = c(0, 100), oob = scales::squish, alpha = 0.85) + p
```


**Ext.Data.Fig.3d: **Correlation between stop types for each drug:
...
**Fig.3e:** Gene-specificity of readthrough drugs

  What we want to answer here is whether the percentage of PTCs whose readthrough is maximized by each of the drugs changes across genes? Generate the 'topdrugs_pergene' datatable:
  For each mutation in our dataset, calculate which is the treatment maximises readthrough:
```
      topdrugs_pergene<-treated_samples[!is.na(RT) & replicate==1 & reads_allbins>=10, max(RT), by=c("mutation_identifier", "stop_type")]
...
``` 
Alternatively, you can load the 'topdrugs_pergene.rds' object from base_dir:
...
ggplot(topdrugs_pergene[treatment!="Gentamicin" & !(GENEINFO=="RAD50" & treatment=="FUr")], aes(x = as.character(GENEINFO), y=perc, fill=treatment)) +
      ...
      pdf("C:/Users/itoledano/Desktop/Readthrough/disease_PTCs/Paper_figures/fig.3/fig3_suppl_bestdrug_pergene.pdf", width = 13.29, height = 5.24)
      print(p1); dev.off()  
...
**Ext.Data.Fig.3f:** readthrough efficiencies for all PTEN mutations measured in our dataset. Each drug-specific plot is stored in p1,p2,...,p9.

```
    p1<-ggplot(treated_samples[treatment=="Clitocine" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
    ...
    p2<-ggplot(treated_samples[treatment=="SRI" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
    ...
    p3<-ggplot(treated_samples[treatment=="G418" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
    ...
    p4<-ggplot(treated_samples[treatment=="DAP" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
    ...
    p5<-ggplot(treated_samples[treatment=="SJ6986" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
    ...
    p6<-ggplot(treated_samples[treatment=="CC90009" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() +
    ...
    p7<-ggplot(treated_samples[treatment=="FUr" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() + 
    ...
    p8<-ggplot(treated_samples[treatment=="Gentamicin" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() + 
    ...
    p9<-ggplot(treated_samples[treatment=="Untreated" & replicate==1 & GENEINFO=="PTEN" & reads_allbins>=10], aes(x=aa_gene_position, y=RT, group=toupper(stop_type), color=toupper(stop_type)))+ geom_point() + 
...
**Ext.Data Fig.3g:** IDUA and ATM genes
...
**Ext.Data Fig.3i:** Ataluren linical trials
...
**Ext.Data Fig.3j,k:** Patient responses

load the following object from base_dir:

```
  rev1_G418<-readRDS(file = file.path(base_dir, "./rev1_G418.rds"))
  rev1_SJ6986<-readRDS(file = file.path(base_dir, "./rev1_SJ6986.rds"))
  rev2_G418b<-readRDS(file = file.path(base_dir, "./rev2_G418b.rds"))
  rev2_SJ6986b<-readRDS(file = file.path(base_dir, "./rev2_SJ6986b.rds"))
...
p1<-ggplot(rev1_SJ6986, aes(x=response, y=RT)) + geom_point(color='black', size=5) + geom_smooth(method='lm', se = FALSE, color="red") + stat_cor(method = "pearson", size=7) + ggtitle("SJ6986") + 
          p + theme(axis.title.y=element_blank(), axis.title.x=element_blank(), plot.margin = margin(0.2, 0.2, 1, 0.2, "cm"))
        p2<-ggplot(rev1_G418, aes(x=response, y=RT)) + geom_point(color='black', size=5) + geom_smooth(method='lm', se = FALSE, color="red") + stat_cor(method = "pearson", size=7) + ggtitle("G418") + 
           p + theme(axis.title.y=element_blank(), axis.title.x=element_blank(), plot.margin = margin(0.2, 0.2, 1, 0.2, "cm"))
        
        p3<-ggarrange(p2, p1, nrow=2, ncol=1)
        annotate_figure(p3, left = textGrob("DMS Readthrough (%)", rot = 90, vjust = .5, gp = gpar(fontfamily="Helvetica", fontsize = 20)),
                        bottom = textGrob("Response to treatment", rot = 0, vjust = .25, gp = gpar(fontfamily="Helvetica", fontsize = 20)),
                        top = textGrob("NCT00458341", rot = 0, vjust = .25, gp = gpar(fontfamily="Helvetica", fontsize = 20, fontface = "bold")))
   
   
   
   
p4<-ggplot(rev2_G418b[Ataluren_concentration==20], aes(y=Quantitative_response, x=RT)) + ylim(0,0.5) + geom_point(color='black', size=5) + geom_smooth(method='lm', se = FALSE, color="red") + stat_cor(method = "pearson", size=7) + ggtitle("G418") + 
          p + theme(axis.title.y=element_blank(), axis.title.x=element_blank(), plot.margin = margin(0.2, 1, 1, 0.2, "cm"))
        p5<-ggplot(rev2_G418b[Ataluren_concentration==20], aes(x=Qualitative_response, y=RT)) + geom_boxplot() + geom_point(color='black', size=5) + stat_compare_means(method = "t.test", size=7) + ggtitle("G418") + 
          p + theme(axis.title.y=element_blank(), axis.title.x=element_blank(), plot.margin = margin(0.2, 0.2, 1, 0.2, "cm"))
        
        p6<-ggplot(rev2_SJ6986b[Ataluren_concentration==20], aes(y=Quantitative_response, x=RT)) + ylim(-0,0.5) + geom_point(color='black', size=5) + geom_smooth(method='lm', se = FALSE, color="red") + stat_cor(method = "pearson", size=7) + ggtitle("SJ6986") + 
           p + theme(axis.title.y=element_blank(), axis.title.x=element_blank(), plot.margin = margin(0.2, 1, 1, 0.2, "cm"))
        p7<-ggplot(rev2_SJ6986b[Ataluren_concentration==20], aes(x=Qualitative_response, y=RT)) + geom_boxplot() + geom_point(color='black', size=5) + stat_compare_means(method = "t.test", size=7) + ggtitle("SJ6986") + 
          p + theme(axis.title.y=element_blank(), axis.title.x=element_blank(), plot.margin = margin(0.2, 0.2, 1, 0.2, "cm"))

        p8<-ggarrange(p4, p6, nrow=2, ncol=1)
        p9<-annotate_figure(p8, bottom = textGrob("Quantitative response", rot = 0, vjust = .5, gp = gpar(fontfamily="Helvetica", fontsize = 20)))
        p10<-ggarrange(p5, p7, nrow=2, ncol=1)
        p11<-annotate_figure(p10, bottom = textGrob("Qualitative response", rot = 0, vjust = .5, gp = gpar(fontfamily="Helvetica", fontsize = 20)))
        ggarrange(p9, p11, nrow=1, ncol=2)
