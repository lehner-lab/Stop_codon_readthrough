...
```
     ...
     set.seed(721); training.samples_1 <- treated_samples[treatment==drugs[j] & replicate==2 & reads_allbins>15 & viral=="no" & !is.na(RT), RT_binomial] %>%
      ...
      all_drugs_predictions<-as.data.table(all_drugs_predictions)
```
...
```
  ...
  model_FUr<-readRDS(file = file.path(base_dir, "./model_FUr.rds"))
    ...
    all_drugs_predictions<-readRDS(file = file.path(base_dir, "./all_drugs_predictions.rds"))
```
Plot:

```
ggplot(all_drugs_predictions[!treatment%chin%c("Untreated", "FUr", "Gentamicin")]) + geom_hex(aes(y=observed, x=predicted), bins=70) + scale_fill_viridis(limits = c(0, 50), oob = scales::squish, alpha = 0.85) + labs(y="Observed", x="Predicted") + stat_cor(aes(y=observed, x=predicted, label = ..rr.label..), size=4, label.y.npc=c(1,1)) +
...
**Fig.4b:** Contribution to model performance of the 8nts downstream of the PTC.
...
```
increase_r2s_ntposition<-readRDS(file = file.path(base_dir, "./increase_r2s_ntposition.rds"))
```
...
**Fig.4c:** General drug model (merging all datapoints from Clitocine, CC90009, DAP, G418, SJ6986 and SRI). 3 different model versions.
...
```
general_model_drugagnostic_6drugs<-readRDS(file = file.path(base_dir, "./general_model_drugagnostic_6drugs.rds"))
...
```
general_model_drugterm_6drugs<-readRDS(file = file.path(base_dir, "./general_model_drugterm_6drugs.rds"))
```
...
```
  general_model_druginteractions_6drugs<-readRDS(file = file.path(base_dir, "./general_model_druginteractions_6drugs.rds"))
```
...
```
  all_pandrugmodels_predictions<-readRDS(file = file.path(base_dir, "./all_pandrugmodels_predictions.rds"))
```
...
```     
Calculate the ratio between the r2 upon term removal and the full-model r2, and then substract it to 100. It represents the % drop in r2 upon term removal\n(normalized to the full model):
```  
      decrease_r2s_drugspec[, r2_perc := 100 - (r2 / r2[Term == "full_model"])*100, by = Treatment]
```   

Alternatively, you can load the 'decrease_r2s_drugspec.rds' object from base_dir:

```
decrease_r2s_drugspec<-readRDS(file = file.path(base_dir, "./decrease_r2s_drugspec.rds"))
```


Plot it:
``` 
  ggplot(decrease_r2s_drugspec[!Treatment%chin%c("Untreated", "FUr", "Gentamicin") & Term!="full_model"], aes(x=Term, y=r2_perc, group=Treatment, color=Treatment, label=round(r2_perc,0))) + geom_point(size=5) + geom_text(nudge_x = .25, size=5)+ labs(x="Model terms", y="% drop in r2\n(r2 upon term removal)/(r2 full model)") + 
            ylim(0,100) + drugs_colors + p + theme(legend.position = "right", legend.text=element_text(size=15), text = element_text(size = 20), axis.text.x = element_text(size = 15, angle=90, vjust = 0.5, hjust=1))
...
```
without_int_2<-readRDS(file = file.path(base_dir, "./without_int_2.rds"))
...
**Extended Data Fig.4a:** Run the code for Fig.4a


**Extended Data Fig.4b:** ANOVA test on the pan drug model.
...
Calculate the ratio between the r2 upon term removal and the full-model r2, and then substract it to 100. It represents the % drop in r2 upon term removal\n(normalized to the full model):
```    
    decrease_r2s_genmodel[, r2_perc := 100 - (r2 / r2[Term == "full_model"])*100]
```    

Alternatively, you can load the 'decrease_r2s_genmodel.rds' object from base_dir:

```
  decrease_r2s_genmodel<-readRDS(file = file.path(base_dir, "./decrease_r2s_genmodel.rds"))
...
```
ggplot(decrease_r2s_genmodel, aes(x=factor(Term, level=c('treatment', 'stop_type', 'down_123nt', 'treatment*stop_type', 'up_123nt', 'treatment*down_123nt', 'treatment*up_123nt', 'treatment*\nstop_type*down_123nt', 'stop_type*down_123nt', 'full_model')), y=r2, group=1, label=round(r2,2))) + geom_point(size=2) + geom_line(size=1, color="#FF6666") + geom_text(nudge_y =0.03)+ labs(x="Model terms", y="Model r2") +  ylim(0,.92) + p + theme(text = element_text(size = 15), axis.text.x = element_text(size = 12, angle=90, vjust = 0.5, hjust=1))
            
```


**Extended Data Fig.4c-e:** visualisation of the coefficients of the drug-specific models.

Load the 'without_int' dtbl generated for Fig.4e.

```
  without_int<-readRDS(file = file.path(base_dir, "./without_int.rds"))
...
**Extended Data Fig.4f:** visualisation of the coefficients of the pan drug model.
...
``` 
  all_coeffs_genmod_withoutint<-readRDS(file = file.path(base_dir, "./all_coeffs_genmod_withoutint.rds"))
``` 
 Plot: 
...
**Extended Data Fig.4g:** r2 increase upon 'stop_type*down_123nt' interaction term inclusion in the model.
...
**Extended Data Fig.4h:** Lasso regularisation models
...
**Extended Data Fig.4i:** encoding the upstream and downstream sequence as a trinucleotide outperforms encoding it as three separate nucleotides (data stored in 'model123_df').
...
```
model123_df<-readRDS(file = file.path(base_dir, "./model123_df.rds"))
```
...
**Extended Data Fig.4j:** Effects of downsampling RT+ variants on model performance. Is it true that if we downsample the number of readthrough+ variants (RT>1%) in drugs with good performing models, the r2 drops?
We rerun the models (10 crossvalidation rounds) by retaining either 20, 50, 150, 300 or 500 variants with RT>1% for DAP, G418, Clitocine, SJ6986, SRI (the drugs with r2>0.7).
As a control, we drop the same number of variants (but randomly, not targeting only variants with RT>1%) and run the models. This is to control for the drop in r2 due to a smaller sized dataset.
...
``` 
    set.seed(721); drugs<-c("DAP", "G418", "Clitocine", "SJ6986", "SRI")
    llista<-list() 
     vars_to_retain<-c(20, 50, 150, 200, 500)
     for (z in 1:length(drugs)){
       if (z<4){ threshold=0.01} else {threshold=0.02} # for DAP, G418 and Clitocine we use the 1% threshold. For SJ6986 and SRI, we use the 2%.
       llista[[z]]<-list()
       for (j in 1:length(vars_to_retain)){
         # Run models when removing RT>1% variants:
         tmp<-treated_samples[treatment==drugs[z] & replicate==2 & reads_allbins>15 & replicate==2 & viral=="no" & !is.na(RT)]   
         tmp<-rbind(tmp[RT_binomial<=threshold], tmp[RT_binomial>threshold][sample(.N, vars_to_retain[j]),])
         set.seed(721); training.samples_1 <- tmp[treatment==drugs[z] & replicate==2 & reads_allbins>15 & replicate==2 & viral=="no" & !is.na(RT), RT_binomial] %>% 
           createDataPartition(p = 0.9, list = FALSE, times=10)
         R2_highRT=c()
         all_predictions<-data.frame()
         for (i in 1:ncol(training.samples_1)){
           train.data_1  <- tmp[treatment==drugs[z] & replicate==2 & reads_allbins>15 & viral=="no" & !is.na(RT)][training.samples_1[,i], ]
           test.data_1 <-  tmp[treatment==drugs[z]& replicate==2 & reads_allbins>15 & viral=="no" & !is.na(RT)][-training.samples_1[,i], ]
           model_1 <- glm(RT_binomial ~ 0 + stop_type + down_123nt + up_123nt + stop_type:down_123nt, train.data_1, family="binomial")
           predictions_1 <- predict(model_1, test.data_1, type = "response")
           R2_highRT = c(R2_highRT, cor(predictions_1, test.data_1$RT_binomial)^2)
         }
         # Now calculate it again, but instead of specifically removing RT>1% variants, randomly remove the same number of variants as a control:
         a<-nrow(tmp)
         tmp<-treated_samples[treatment==drugs[z] & replicate==2 & reads_allbins>15 & replicate==2 & viral=="no" & !is.na(RT)][sample(.N, a)]  
         set.seed(721); training.samples_1 <- tmp[treatment==drugs[z] & replicate==2 & reads_allbins>15 & replicate==2 & viral=="no" & !is.na(RT), RT_binomial] %>% 
           createDataPartition(p = 0.9, list = FALSE, times=10)
         R2_randomcontrol=c()
         all_predictions<-data.frame()
         for (i in 1:ncol(training.samples_1)){
           ...
           model_1 <- glm(RT_binomial ~ 0 + stop_type + down_123nt + up_123nt + stop_type:down_123nt, train.data_1, family="binomial")
           predictions_1 <- predict(model_1, test.data_1, type = "response")
           R2_randomcontrol = c(R2_randomcontrol, cor(predictions_1, test.data_1$RT_binomial)^2)
         }
         llista[[z]][[j]]<-c(mean(R2_highRT), mean(R2_randomcontrol))
         print(j)}
       print(z)}
     
     names(llista)<-drugs
     for (i in 1:length(llista)){names(llista[[i]])<-c("20", "50", "150", "200", "500")}
```
Convert the list into a datatable, named 'downsampling_dtbl': 
```
     downsampling_dtbl<-data.frame(llista)
     downsampling_dtbl<-as.data.table(gather(downsampling_dtbl, treatment, r2, DAP.20:SRI.500, factor_key=TRUE))
     downsampling_dtbl$treatment<-as.character(downsampling_dtbl$treatment)
     downsampling_dtbl[, n_vars:=unlist(strsplit(treatment, "\\."))[2], by=1:nrow(downsampling_dtbl)]
     downsampling_dtbl[, treatment:=unlist(strsplit(treatment, "\\."))[1], by=1:nrow(downsampling_dtbl)]
     downsampling_dtbl[, downsampling_strategy:=rep(c("RT>1%", "random"), nrow(downsampling_dtbl)/2)]
     downsampling_dtbl$downsampling_strategy<-factor(downsampling_dtbl$downsampling_strategy, levels=c("RT>1%", "random"))  
     downsampling_dtbl$n_vars<-factor(downsampling_dtbl$n_vars, levels=c("20", "50", "100", "150", "200", "300", "500"))  
```