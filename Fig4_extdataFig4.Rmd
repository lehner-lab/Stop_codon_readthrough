...
Using the saveRDS function models can be stored in your base_dir. They will be needed again for Fig.6.
...
```
     ...
     set.seed(721); training.samples_1 <- treated_samples[treatment==drugs[j] & replicate==2 & reads_allbins>15 & viral=="no" & !is.na(RT), RT_binomial] %>%
      ...
      for (i in 1:ncol(training.samples_1)){
        ...
        saveRDS(model_1, file = paste0("~/R/RT/Disease_PTCs/Disease_objects/Modeling/Drug_specific_models/model_nt123_", drugs[j],".rds"))
...
```
  ...
  all_drugs_predictions<-readRDS(file = "/base_dir/Fig4/Models/Drug_specific_models/all_drugs_predictions.rds")
```
Plot:
...
```
  general_model_drugagnostic_6drugs<-readRDS(file = "/base_dir/Fig4/Models/Pan_drug_models/general_model_drugagnostic_6drugs.rds")
```
...
```
  general_model_drugterm_6drugs<-readRDS(file = "/base_dir/Fig4/Models/Pan_drug_models/general_model_drugterm_6drugs.rds")
```
...
```
  general_model_druginteractions_6drugs<-readRDS(file = "/base_dir/Fig4/Models/Pan_drug_models/general_model_druginteractions_6drugs.rds")
```
...
```
  all_pandrugmodels_predictions<-readRDS(file = "/base_dir/Fig4/Models/Pan_drug_models/all_pandrugmodels_predictions.rds")
```
...
```
  increase_r2s_ntposition<-readRDS(file = "/base_dir/Fig4/Models/Drug_specific_models/increase_r2s_ntposition.rds")
```
...
```
  decrease_r2s_drugspec<-readRDS(file = "/base_dir/Fig4/Models/Drug_specific_models/decrease_r2s_drugspec.rds")
```
...
```
  without_int_2<-readRDS(file = "/base_dir/Fig4/Models/Drug_specific_models/without_int_2.rds")
```
...
```
  decrease_r2s_genmodel<-readRDS(file = "/base_dir/Fig4/Models/Pan_drug_models/decrease_r2s_genmodel.rds")
```
...
```
  increase_r2s_drugspec_interactions<-readRDS(file = "/base_dir/Fig4/Models/Drug_specific_models/increase_r2s_drugspec_interactions.rds")
```
...
```
  model123_df<-readRDS(file = "/base_dir/Fig4/Models/model123_df.rds")
```
...
```
  without_int<-readRDS(file = "/base_dir/Fig4/Models/Drug_specific_models/without_int.rds")
```
...
``` 
        all_coeffs_genmod_withoutint<-readRDS(file = "/base_dir/Fig4/Models/Pan_drug_models/all_coeffs_genmod_withoutint.rds")
``` 
 Plot: 
